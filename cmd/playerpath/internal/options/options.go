...
func Init() *Options {
	...
	flag.TextVar(&opts.Provider, "provider", provider.BF2Hub, "provider to use as fallback if one cannot be selected based on player/server (bf2hub|playbf2|openspy|b2bf2)")
	flag.Parse()
	return opts
}
